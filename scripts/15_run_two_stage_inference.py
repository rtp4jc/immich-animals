...
from dog_id.common.visualization import setup_output_dir
...
def find_closest_keypoint_instance(keypoint_results, original_bbox):
    ...
    for i, box in enumerate(keypoint_results[0].boxes.xyxy):
        ...
        distance = ((kp_center_x - orig_center_x) ** 2 + (kp_center_y - orig_center_y) ** 2) ** 0.5
...
def visualize_results(image, detections, keypoint_results, crop_offsets, output_path):
    ...
    for i, (det_box, kp_result, crop_offset) in enumerate(zip(detections, keypoint_results, crop_offsets)):
        # Draw detection bounding box
        x1, y1, x2, y2 = map(int, det_box)
        cv2.rectangle(vis_image, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(vis_image, f'Dog {i+1}', (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        ...
        if kp_result and len(kp_result[0].keypoints) > 0:
            ...
            for j, (kp, name, color) in enumerate(zip(keypoints, kp_names, colors)):
                if len(kp) >= 2:
                    ...
                    cv2.putText(vis_image, name, (orig_x+5, orig_y-5), 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run two-stage dog detection and keypoint inference")
    parser.add_argument("--detector-prefix", default="detector_yolo11n", 
                       help="Detector model run prefix")
    parser.add_argument("--keypoint-prefix", default="keypoint_yolo11n", 
                       help="Keypoint model run prefix")
    parser.add_argument("--input-dir", default="data/sample_images", 
                       help="Directory containing input images")
    parser.add_argument("--output-dir", default="outputs/15_two_stage_inference",
        ...
        for box in det_results[0].boxes.xyxy:
            ...
            if len(kp_results[0].boxes) > 1:
                ...
                if closest_idx is not None:
                    ...
                    kp_results[0].boxes = kp_results[0].boxes[closest_idx:closest_idx+1]
                    kp_results[0].keypoints = kp_results[0].keypoints[closest_idx:closest_idx+1]
            
            keypoint_results.append(kp_results)
        
        # Visualize and save results
        output_path = output_dir / f"result_{img_file.name}"
        visualize_results(image, detections, keypoint_results, crop_offsets, output_path)
