#!/usr/bin/env python
"""
Export Script for YOLOv11 Keypoint Model to ONNX

This script loads the best-performing keypoint model from the Phase 1 training
...
def main():
    ...
    print("--- YOLOv11 Keypoint Model ONNX Exporter ---")

    # --- Find latest training run ---
    print(f"Searching for latest run in: {KEYPOINT_PROJECT_DIR}")
    latest_run_dir = find_latest_run(KEYPOINT_PROJECT_DIR, KEYPOINT_RUN_NAME)
    if not latest_run_dir:
        print(f"[ERROR] No training runs found for '{KEYPOINT_RUN_NAME}' in '{KEYPOINT_PROJECT_DIR}'.")
        sys.exit(1)
    ...
    if not model_checkpoint.exists():
        ...
        print("Please ensure that the keypoint model has been trained and the path is correct.")
        sys.exit(1)
    ...
    ONNX_KEYPOINT_PATH.parent.mkdir(parents=True, exist_ok=True)
    ...
    if ONNX_KEYPOINT_PATH.exists():
        print(f"Verified: ONNX model created at {ONNX_KEYPOINT_PATH}")
        print(f"File size: {ONNX_KEYPOINT_PATH.stat().st_size / 1024 / 1024:.2f} MB")
    else:
