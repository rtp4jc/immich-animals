...
class ImmichAnimalSystem(AnimalIdentificationSystem):
    ...
    def __init__(self, host: str = "localhost", port: int = 3003, model_config: dict = None):
        self.host = host
        self.port = port
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark animal identification pipeline via Immich API")
    ...
    if args.num_queries > 0:
        ...
        if valid_queries:
            ...
            output_dir = "outputs/immich_with_keypoints" if not args.skip_keypoints else "outputs/immich_without_keypoints"
