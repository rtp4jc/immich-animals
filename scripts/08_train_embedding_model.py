...
from animal_id.embedding.config import DEFAULT_BACKBONE, TRAINING_CONFIG, DATA_CONFIG
...
def main():
    ...
    final_results = {
        ...
        "train_metrics": {
            k: float(v) if hasattr(v, "item") else v for k, v in train_metrics.items()
        },
        "val_metrics": {
            k: float(v) if hasattr(v, "item") else v for k, v in val_metrics.items()
        },
        "backbone": DEFAULT_BACKBONE.value,
        "config": {"training": TRAINING_CONFIG, "data": DATA_CONFIG},
    }
