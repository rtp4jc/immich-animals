...
from dog_id.benchmark.evaluator import BenchmarkEvaluator
...
class ONNXDetector:
    ...
    def predict(self, image: np.ndarray):
        ...
        detections = self.session.run(None, {self.session.get_inputs()[0].name: detector_input})[0][0]
...
class ONNXKeypoint:
    ...
    def predict(self, image: np.ndarray):
        ...
        detections = self.session.run(None, {self.session.get_inputs()[0].name: keypoint_input})[0][0]
...
class ONNXEmbedding:
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        embedding = self.session.run(None, {self.session.get_inputs()[0].name: embedding_input})[0][0]
        return embedding
...
def main(args):
    ...
    missed_with_kp = sum(1 for r in evaluator.get_results() if r.has_animal_gt and not r.has_animal_pred)
    if missed_with_kp > 0:
        print(f"\n--- {missed_with_kp} images with no dogs detected (with keypoints) ---")
        missed_examples = [r for r in evaluator.get_results() if r.has_animal_gt and not r.has_animal_pred][:10]
    ...
    if args.num_queries > 0:
        ...
        valid_queries = [item['image_path'] for item in ground_truth if item['identity_label']]
        if valid_queries:
            num_queries = min(args.num_queries, len(valid_queries))
            query_indices = np.linspace(0, len(valid_queries) - 1, num_queries, dtype=int)
            query_images = [valid_queries[i] for i in query_indices]
            
            visualizer.visualize_queries(query_images, "outputs/ambidextrous_axolotl", display=False)
            visualizer.plot_metrics_summary("outputs/ambidextrous_axolotl", display=False)
