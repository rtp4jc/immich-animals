...
class CocoDetectorDatasetConverter:
    ...
    def _load_coco_bbox_only(self, json_path, split_name, num_negatives, image_id_counter):
        ...
        for img_id, anns in annotations_by_image.items():
            ...
            temp_id = image_id_counter
            image_id_counter += 1
            pos_images.append({'id': temp_id, 'file_name': f"coco/images/{split_name}/{img['file_name']}", 'width': width, 'height': height})
            ...
            for ann in anns:
                ...
                bbox = [round(x1, 2), round(y1, 2), round(x2 - x1, 2), round(y2 - y1, 2)]
                if bbox[2] <= 0 or bbox[3] <= 0: continue
                pos_annotations.append({'id': -1, 'image_id': temp_id, 'category_id': 1, 'bbox': bbox, 'area': bbox[2] * bbox[3], 'iscrowd': ann['iscrowd']})
        ...
        for img in neg_images_sampled:
            ...
            image_id_counter += 1
        neg_images = [{'id': img['id'], 'file_name': f"coco/images/{split_name}/{img['file_name']}", 'width': img['width'], 'height': img['height']} for img in neg_images_sampled]
        
        return pos_images, pos_annotations, neg_images, image_id_counter

    def _load_oxford_pets_negatives(self, xml_dir, image_id_counter):
        ...
        for xml_path in tqdm(list(Path(xml_dir).glob("*.xml")), desc="Processing Oxford Negatives"):
            try:
                ...
                if root.find('object/name').text != 'dog':
                    ...
                    temp_id = image_id_counter
                    image_id_counter += 1
                    images.append({'id': temp_id, 'file_name': f"oxford_pets/images/{img_filename}", 'width': int(size.find('width').text), 'height': int(size.find('height').text)})
            except (ET.ParseError, AttributeError):
                continue
        return images, [], image_id_counter

    def _finalize_annotations(self, annotations, annotation_id_counter):
    ...
    def convert(self):
        ...
        sb_images, sb_annotations, image_id_counter = self._load_stanford_base_bboxes(self.config['stanford_base_dir'], set(), image_id_counter)
        datasets_to_split.append(("Stanford Base", sb_images, sb_annotations))

        print("Loading Oxford Pets Negatives...")
        op_images, op_annotations, image_id_counter = self._load_oxford_pets_negatives(self.config['oxford_xml_dir'], image_id_counter)
        ...
        coco_train_pos_imgs, coco_train_pos_anns, coco_train_neg_imgs, image_id_counter = self._load_coco_bbox_only(self.config['coco_train_json'], 'train2017', 15000, image_id_counter)
        ...
        coco_val_pos_imgs, coco_val_pos_anns, coco_val_neg_imgs, image_id_counter = self._load_coco_bbox_only(self.config['coco_val_json'], 'val2017', 2000, image_id_counter)
