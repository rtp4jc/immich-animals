#!/usr/bin/env python3
"""
Creates a sample_images directory for the detector from its validation set.

This script selects a small number of images from the detector's validation set
(as defined in data/detector/coco/annotations_val.json) and copies them to
outputs/phase1/detector_sample_images/. These images are intended for quick visual
inspection of the detector model's inference.
"""
...
# Use the validation set from the new detector dataset
COCO_ANNOTATIONS_VAL = DATA_ROOT / "detector" / "coco" / "annotations_val.json"
# Output to the new directory for detector samples
SAMPLE_IMAGES_DIR = REPO_ROOT / "outputs" / "phase1" / "detector_sample_images"

NUM_SAMPLES_PER_DATASET = 10 # Number of samples to take from each source dataset

def main():
    print("=" * 60)
    print("Creating sample images from the detector validation set")
    print("=" * 60)

    if not COCO_ANNOTATIONS_VAL.exists():
        print(f"Error: Detector validation annotations file not found: {COCO_ANNOTATIONS_VAL}")
        print("Please run 'python scripts/phase1/create_detector_coco_dataset.py' first.")
        return
    ...
    if not val_images_info:
        print(f"Error: No image information found in {COCO_ANNOTATIONS_VAL}.")
        return
