...
- **`animal_id/`** - Existing package structure and modules
...
2. **Centralize Core Logic**: Extract reusable logic into `animal_id` package modules
...
**Task**: Create the missing package infrastructure for the `animal_id` package.

**Requirements**:

- Create `animal_id/__init__.py`
- Create `animal_id/detection/` module with `__init__.py`
- Create `animal_id/keypoint/` module with `__init__.py`
- Create placeholder files: `animal_id/common/visualization.py` and `animal_id/common/inference.py`

**Explicit Acceptance Criteria**:

- [✅] Run `python -c "import animal_id; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.detection import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.keypoint import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.common.visualization import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.common.inference import *; print('Success')"` - must print "Success"
...
**Task**: Extract core logic from Phase 1 detection scripts into reusable modules within `animal_id/detection/`.

**Requirements**:

- Extract `CocoDetectorDatasetConverter` class to `animal_id/detection/dataset_converter.py`
- Extract COCO to YOLO conversion logic to `animal_id/detection/yolo_converter.py`
- Extract training configuration and logic to `animal_id/detection/trainer.py`
- Extract validation logic to `animal_id/detection/validator.py`
- Maintain all existing functionality and configuration options
- Add proper imports and error handling

**Explicit Acceptance Criteria**:

- [✅] Run `python -c "from animal_id.detection.dataset_converter import CocoDetectorDatasetConverter; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.detection.yolo_converter import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.detection.trainer import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.detection.validator import *; print('Success')"` - must print "Success"
...
**Task**: Extract core logic from Phase 1 keypoint scripts into reusable modules within `animal_id/keypoint/`.

**Requirements**:

- Extract keypoint dataset creation logic to `animal_id/keypoint/dataset_converter.py`
- Extract COCO to YOLO conversion logic to `animal_id/keypoint/yolo_converter.py`
- Extract training configuration and logic to `animal_id/keypoint/trainer.py`
- Maintain all existing functionality and configuration options
- Add proper imports and error handling

**Explicit Acceptance Criteria**:

- [✅] Run `python -c "from animal_id.keypoint.dataset_converter import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.keypoint.yolo_converter import *; print('Success')"` - must print "Success"
- [✅] Run `python -c "from animal_id.keypoint.trainer import *; print('Success')"` - must print "Success"
...
**Task**: Create centralized visualization utilities in `animal_id/common/visualization.py` that can be reused across all scripts.
...
**Background**: Review `.planning/long-term-folder-structure.md` and the extracted `animal_id.detection` modules from previous prompts.

**Task**: Create `01_prepare_detection_data.py` that creates and converts detection dataset without visualization.

**Requirements**:

- Use `animal_id.detection.dataset_converter` to create COCO dataset
- Use `animal_id.detection.yolo_converter` to convert to YOLO format
...
- Update existing `visualize_generic_dataset.py` to use `animal_id.common.visualization`
...
- Use `animal_id.detection.trainer` for training configuration and execution
- Use `animal_id.detection.validator` for model validation
- Use `animal_id.common.visualization` to visualize training results and validation outputs
...
**Background**: Review `.planning/long-term-folder-structure.md` and the extracted `animal_id.keypoint` modules from previous prompts.

**Task**: Create `04_prepare_keypoint_data.py` that creates and converts keypoint dataset without visualization.

**Requirements**:

- Use `animal_id.keypoint.dataset_converter` to create COCO keypoint dataset
- Use `animal_id.keypoint.yolo_converter` to convert to YOLO format
- No visualization (users will run script 02 for inspection)
...
- Use `animal_id.keypoint.trainer` for training configuration and execution
- Use `animal_id.common.visualization` to visualize training results and keypoint predictions
...
- Update script to use `animal_id.common.visualization` and `animal_id.common.inference` where beneficial
