...
|---|----------|-------|--------|
| 1 | **Fix TAR@FAR threshold range bug** — replace binary search with `np.quantile`, unify the two implementations | `animal_id/benchmark/evaluator.py:305-329`, `animal_id/benchmark/metrics.py:108-157` | bench F1/F4 |
| 2 | **Fix TAR@FAR pair duplication** — stop counting both `(A,B)` and `(B,A)` | `animal_id/benchmark/evaluator.py:273-285` | bench F2 |
