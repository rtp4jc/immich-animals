...
def mock_trainer_with_empty_val_loader(tmp_path):
    ...
    with patch('animal_id.embedding.trainer.evaluate_embedding_model', return_value={}) as mock_eval:
...
def test_train_loop_handles_empty_val_metrics(mock_trainer_with_empty_val_loader):
    ...
    try:
        ...
        pytest.fail(f"Training loop crashed with KeyError, likely from accessing val_metrics: {e}")
