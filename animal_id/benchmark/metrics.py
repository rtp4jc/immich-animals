...
def _generate_positive_pairs(labels: np.ndarray, labels_to_indices: Dict[Any, List[int]]) -> List[Tuple[int, int]]:
...
def _generate_negative_pairs(
    labels: np.ndarray,
    labels_to_indices: Dict[Any, List[int]],
    ...
    if n_samples < 500:
        ...
        if len(all_possible_negatives) > target:
            indices = np.random.choice(len(all_possible_negatives), target, replace=False)
...
def _calculate_pair_scores(embeddings: torch.Tensor, pairs: List[Tuple[int, int]]) -> np.ndarray:
...
def calculate_tar_at_far(embeddings: np.ndarray, labels: np.ndarray, far_threshold: float = 0.01, seed: int = 42) -> Tuple[float, float]:
    """
    ...
    negative_pairs = _generate_negative_pairs(labels, labels_to_indices, num_target_negatives)
