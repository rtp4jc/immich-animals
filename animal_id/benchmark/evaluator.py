    ...
    def __str__(self) -> str:
        ...
        if self.tar_at_far:
            ...
            for far, (tar, threshold) in sorted(self.tar_at_far.items()):
                lines.append(
                    f"  FAR={far * 100:5.1f}%: TAR={tar:.3f} ({tar * 100:.1f}%) @ threshold={threshold:.4f}"
                )
...
class BenchmarkEvaluator:
    ...
    def evaluate(self, system: DogIdentificationSystem) -> BenchmarkMetrics:
        ...
        all_image_paths = [
            str(self.data_root / item["image_path"]) for item in self.ground_truth
        ]
        ...
        for item in tqdm(self.ground_truth, desc="Evaluating images"):
            ...
            if has_animal_gt and has_animal_pred and identity_gt:
                ...
                for rank, (sim_path, _) in enumerate(similar_images, 1):
                    ...
                    if (
                        sim_rel_path in same_identity_images
                        and sim_rel_path != item["image_path"]
                    ):
                        identity_rank = rank
                        break
    ...
    def _compute_metrics(self) -> BenchmarkMetrics:
        ...
        detection_accuracy = (
            detection_correct / total_images if total_images > 0 else 0.0
        )

        true_positives = sum(
            1 for r in self.results if r.has_animal_gt and r.has_animal_pred
        )
        false_positives = sum(
            1 for r in self.results if not r.has_animal_gt and r.has_animal_pred
        )
        false_negatives = sum(
            1 for r in self.results if r.has_animal_gt and not r.has_animal_pred
        )
        ...
        valid_ranks = [
            r.identity_rank for r in self.results if r.identity_rank is not None
        ]
        mean_reciprocal_rank = (
            np.mean([1.0 / rank for rank in valid_ranks]) if valid_ranks else 0.0
        )
    ...
    def _compute_tar_at_far(self) -> Dict[float, Tuple[float, float]]:
        ...
        detected_results = [
            ...
            if r.has_animal_gt and r.has_animal_pred and r.identity_gt
        ]
