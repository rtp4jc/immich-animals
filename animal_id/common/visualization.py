...
def save_or_show_plot(save_path: Optional[Union[str, Path]] = None, 
                     display: bool = False,
                     dpi: int = 150) -> None:
...
def visualize_coco_annotations(coco_json_path: Union[str, Path], 
...
def visualize_yolo_annotations(data_yaml_path: Union[str, Path],
...
def visualize_keypoint_results(image_path: Union[str, Path],
    ...
    for kpt_data in keypoints:
        ...
        for i in range(0, len(kpts), 3):
            if i//3 < len(keypoint_names) and kpts[i+2] > 0:
                ax.plot(kpts[i], kpts[i+1], 'o', color=colors[i//3], 
                       markersize=8, label=keypoint_names[i//3] if i == 0 else "")
...
def visualize_training_metrics(metrics_data: Dict[str, List[float]],
                             output_path: Optional[Union[str, Path]] = None,
                             display: bool = False) -> None:
...
def visualize_identity_dataset(identity_json_path: Union[str, Path],
                             data_root: Union[str, Path],
    ...
    valid_identities = {k: v for k, v in identity_groups.items() 
                       if len(v) >= min_images_per_id}
    
    if len(valid_identities) < num_identities:
        print(f"Only {len(valid_identities)} identities have >= {min_images_per_id} images")
        num_identities = len(valid_identities)
    
    selected_identities = random.sample(list(valid_identities.keys()), num_identities)
    
    fig, axes = plt.subplots(num_identities, min_images_per_id, 
