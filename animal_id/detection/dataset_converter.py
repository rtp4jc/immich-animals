...
class CocoDetectorDatasetConverter:
    ...
    def _load_stanford_base_bboxes(
        ...
        for xml_path in tqdm(xml_files, desc="Processing Stanford Base"):
            try:
                ...
                relative_path = (
                    f"stanford_dogs/images/{xml_path.parent.name}/{filename_stem}.jpg"
                )
    ...
    def _load_coco_bbox_only(
        ...
        dog_cat_id = next(
            (cat["id"] for cat in data.get("categories", []) if cat["name"] == "dog"),
            -1,
        )
        ...
        neg_images_all = [
            img for img_id, img in all_images_map.items() if img_id not in dog_image_ids
        ]
    ...
    def _load_oxford_pets_negatives(
        ...
        for xml_path in tqdm(
            list(Path(xml_dir).glob("*.xml")), desc="Processing Oxford Negatives"
        ):
            try:
                tree = ET.parse(xml_path)
                root = tree.getroot()
    ...
    def convert(self) -> None:
        ...
        val_annotations, annotation_id_counter = self._finalize_annotations(
            val_annotations, annotation_id_counter
        )
